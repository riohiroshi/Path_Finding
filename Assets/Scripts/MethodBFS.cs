...
public class MethodBFS : IPathFindingMethod
{
    private int[,] _searchingMap;
    private Queue<MapPosition> _searchingQueue;

    private SearchMethod _methodType = SearchMethod.BFS;
    ...
    {
        _searchingQueue = new Queue<MapPosition>();
        ...
        _searchingQueue.Enqueue(pathFinding.StartPoint);

        var checkingPos = _searchingQueue.Dequeue();
        ...
        while (!checkingPos.Equals(pathFinding.EndPoint))
        {
            ...
            checkingPos = _searchingQueue.Dequeue();
        ...
        bool CheckPos(int offsetX, int offsetY)
        {
            ...
            if (pathFinding.OriginMap[nextX, nextY] == PathFinding.POINT_END)
            {
                ...
                _searchingQueue.Clear();
                return true;
            }

            if (pathFinding.OriginMap[nextX, nextY] == PathFinding.POINT_EMPTY && _searchingMap[nextX, nextY] == PathFinding.POINT_EMPTY)
            {
                ...
                _searchingQueue.Enqueue(tempPos);
            }
